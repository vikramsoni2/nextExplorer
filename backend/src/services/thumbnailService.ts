...
import { directories, extensions, thumbnails } from '../config';
...
const EXECUTABLE_CANDIDATES: Record<'ffmpeg' | 'ffprobe', Array<string | undefined>> = {
  ffmpeg: [
...
const resolveExecutable = (candidates: Array<string | undefined>): string | null => {
...
const configureFfmpegBinaries = (): void => {
...
const isImage = (ext: string): boolean => extensions.images.includes(ext);
const isVideo = (ext: string): boolean => extensions.videos.includes(ext);
const isPdf = (ext: string): boolean => ext === 'pdf';

const inflight = new Map<string, Promise<string>>();

const hashForFile = async (filePath: string): Promise<string> => {
...
const atomicWrite = async (finalPath: string, buffer: Buffer): Promise<void> => {
...
const makeImageThumb = async (srcPath: string, destPath: string): Promise<void> => {
...
const probeDuration = (filePath: string): Promise<number | null> =>
  new Promise((resolve) => {
...
const makeVideoThumb = async (srcPath: string, destPath: string): Promise<void> => {
...
const generateThumbnail = async (filePath: string, thumbPath: string): Promise<void> => {
...
const buildThumbnailPaths = async (filePath: string): Promise<{ thumbFile: string; thumbPath: string }> => {
...
export const getThumbnailPathIfExists = async (filePath: string): Promise<string> => {
...
export const queueThumbnailGeneration = (filePath: string): void => {
  ...
  void getThumbnail(filePath).catch((error: unknown) => {
    const message = error instanceof Error ? error.message : 'Unknown error';
    console.warn(`Queued thumbnail generation failed for ${filePath}: ${message}`);
  });
};
