...
import { ArrowLeftIcon, TrashIcon, KeyIcon, ShieldCheckIcon, CloudIcon, ServerIcon, PlusIcon, PencilIcon, FolderIcon } from '@heroicons/vue/24/outline';
...
const emit = defineEmits(['back', 'update', 'reset-password', 'delete', 'make-admin', 'revoke-admin']);
const { t } = useI18n();
  ...
  <div class="flex flex-col h-full animate-fade-in">
    ...
    <div class="flex items-center gap-4 mb-6">
      ...
      <div class="flex items-center gap-4">
        <div class="h-12 w-12 rounded-full bg-zinc-200 dark:bg-zinc-800 flex items-center justify-center text-zinc-600 dark:text-zinc-400 font-medium text-lg">
        ...
        <div>
          ...
          <div class="flex items-center gap-2 text-sm text-zinc-500 dark:text-zinc-400">
            ...
            <span v-if="isAdmin" class="inline-flex items-center rounded-full bg-zinc-100 px-2 py-0.5 text-xs font-medium text-zinc-800 dark:bg-zinc-800 dark:text-zinc-200">
              Admin
            </span>
          </div>
        </div>
    ...
    <div class="flex-1 overflow-y-auto pb-10">
      ...
      <div v-if="activeTab === 'profile'" class="space-y-8 max-w-3xl">
        <div class="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800 p-6">
          <h3 class="text-lg font-medium text-zinc-900 dark:text-zinc-100 mb-4">General Information</h3>
        ...
        <!-- Roles -->
        <div class="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800 p-6">
          <h3 class="text-lg font-medium text-zinc-900 dark:text-zinc-100 mb-4">Roles & Permissions</h3>
        ...
        <!-- Danger Zone -->
        <div v-if="!isCurrentUser" class="bg-red-50 dark:bg-red-900/10 rounded-lg border border-red-200 dark:border-red-900/30 p-6">
      ...
      <div v-if="activeTab === 'security'" class="space-y-8 max-w-3xl">
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800 p-6">
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-1 ml-7">
                {{ hasLocalAuth ? 'User has a local password set.' : 'User does not have a local password set.' }}
              </p>
            </div>
            <button
        ...
        <!-- OIDC Profiles -->
        <div class="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800 p-6">
          <h3 class="text-lg font-medium text-zinc-900 dark:text-zinc-100 mb-4">Single Sign-On (OIDC)</h3>
          
          <div v-if="oidcProfiles.length > 0" class="space-y-4">
             <div v-for="(profile, idx) in oidcProfiles" :key="idx" class="flex items-center justify-between py-3 border-b border-zinc-100 dark:border-zinc-800 last:border-0">
                <div class="flex items-center gap-3">
                   <div class="p-2 bg-zinc-100 dark:bg-zinc-800 rounded-full">
                      <CloudIcon class="w-5 h-5 text-zinc-600 dark:text-zinc-400" />
                   </div>
                   <div>
                      <p class="text-sm font-medium text-zinc-900 dark:text-zinc-100">{{ profile.provider || 'OIDC Provider' }}</p>
                      <p class="text-xs text-zinc-500 dark:text-zinc-400">Linked Profile</p>
                   </div>
                </div>
                <!-- Placeholder for unlink action if needed in future -->
                <span class="text-xs text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/20 px-2 py-1 rounded-full">Active</span>
             </div>
          </div>
      ...
      <div v-if="activeTab === 'volumes'" class="space-y-8 max-w-3xl">
        <div class="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800 p-6">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-medium text-zinc-900 dark:text-zinc-100">{{ t('settings.users.assignedVolumes') }}</h3>
            <button
          ...
          <div v-if="volumesLoading" class="py-8 text-center">
            <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-zinc-900 dark:border-zinc-100 mx-auto"></div>
            <p class="mt-2 text-sm text-zinc-500 dark:text-zinc-400">{{ t('common.loading') }}</p>
          </div>

          <!-- Empty state -->
          <div v-else-if="volumes.length === 0" class="text-center py-8 text-zinc-500 dark:text-zinc-400">
          ...
          <div v-else class="space-y-3">
            ...
            >
              <div class="flex items-center gap-4">
                ...
                <div>
                  <p class="font-medium text-zinc-900 dark:text-zinc-100">{{ volume.label }}</p>
                  <p class="text-xs text-zinc-500 dark:text-zinc-400 font-mono">{{ volume.path }}</p>
                </div>
              </div>
              <div class="flex items-center gap-3">
                <span
        ...
        <!-- Info box -->
        <div class="bg-blue-50 dark:bg-blue-900/10 rounded-lg border border-blue-200 dark:border-blue-900/30 p-4">
