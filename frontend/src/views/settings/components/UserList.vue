...
<template>
  <div class="flex flex-col h-full">
    ...
    <!-- Empty State -->
    <div v-else-if="users.length === 0" class="flex-1 flex flex-col items-center justify-center text-zinc-500">
      <UserCircleIcon class="w-12 h-12 mb-2 opacity-50" />
      <p>{{ t('settings.users.noUsers') }}</p>
    </div>

    <!-- User List -->
    <div v-else class="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800 overflow-hidden">
        ...
        >
          ...
          <div class="flex items-center gap-3">
            <div class="flex -space-x-1 overflow-hidden">
              ...
              >
                <component :is="getAuthIcon(auth.method)" class="w-3.5 h-3.5 text-zinc-500 dark:text-zinc-400" />
              </div>
            </div>
