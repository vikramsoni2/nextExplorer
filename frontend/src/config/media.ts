...
const getImagePreviewExtensions = (): string[] => Array.from(imagePreviewExtensionsSet.values());

const isPreviewableImage = (extension = ''): boolean => {
  if (!extension) return false;
  return imagePreviewExtensionsSet.has(extension.toLowerCase());
};

const getVideoPreviewExtensions = (): string[] => Array.from(videoPreviewExtensionsSet.values());

const isPreviewableVideo = (extension = ''): boolean => {
  if (!extension) return false;
