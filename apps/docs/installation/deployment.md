...
- `PUBLIC_URL` informs the backend's cookie settings, CORS, and default OIDC callback (see `apps/api/src/config/env.js`).
...
- Use `LOG_LEVEL`, `DEBUG`, and `ENABLE_HTTP_LOGGING` (from `apps/api/src/config/env.js`) to tune logging verbosity.
